"""
Migration 0008: 創建 chat_history 表
...
class Migration0008_CreateChatHistory(Migration):
    """創建 chat_history 表"""
    ...
    async def up(self, db) -> None:
        ...
        ''')
        print(f"[Migration 0008] Created chat_history table", file=sys.stderr)
        ...
        print(f"[Migration 0008] Migration completed successfully", file=sys.stderr)
    
    async def down(self, db) -> None:
        """Rollback migration (downgrade)"""
        print(f"[Migration 0008] Dropping chat_history table...", file=sys.stderr)
...
# 註冊遷移
migration = Migration0008_CreateChatHistory()
