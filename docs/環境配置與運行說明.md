...
| 項目 | 值 | 說明 |
|------|-----|------|
| **正式網址** | `https://tgw.usdt2026.cc` | 登錄頁、API、管理後台同一域名 |
| **Telegram Bot** | `@tgzkw_bot` | 掃碼登錄與驗證碼登入，須與 BotFather 創建一致 |
| **Webhook** | `https://tgw.usdt2026.cc/webhook/telegram` | Telegram 回調地址，部署時需在 Bot 後台設置 |
| **前端 defaultLoginApiUrl** | `https://tgw.usdt2026.cc` | 見 `environment.ts` / `environment.prod.ts` |
| **後端 INTERNAL_API_URL** | `https://tgw.usdt2026.cc` | 與上述一致，Bot 查詢 login token 用 |

若更換域名或 Bot，需同時修改：前端環境文件中的 `defaultLoginApiUrl`、後端 `.env` 的 `INTERNAL_API_URL` 與 `TELEGRAM_BOT_USERNAME`、以及 Telegram Bot 的 Webhook URL。
...
## 三、掃碼登錄失敗排查（提示「登錄失敗 / 此二維碼不是由當前服務器生成」）

此錯誤表示 **Bot 調用的後端與生成二維碼的後端不一致**（或未共享同一數據庫）。

1. **確認同一套後端與數據庫**
   - 用戶在瀏覽器打開的登錄頁（如 `https://tgw.usdt2026.cc/auth/login`）會向**該域名的 API** 請求 `POST /api/v1/auth/login-token` 生成 token，並寫入**該 API 使用的數據庫**。
   - 用戶在 Telegram 掃碼後，**Bot** 使用 `INTERNAL_API_URL` 請求 `GET /api/v1/auth/login-token/{token}` 和 `POST .../confirm`。
   - 若 `INTERNAL_API_URL` 指向的地址與用戶打開的登錄頁所屬 API **不是同一個**（或 API 多實例但未共享同一 `login_tokens` 表），就會查不到 token，Bot 回覆「此二維碼不是由當前服務器生成」。

2. **必做**
   - 在**運行 Bot/API 的服務器**上，將 `INTERNAL_API_URL` 設為**對外訪問的 API 基址**（與前端 `defaultLoginApiUrl` 一致），例如：`https://tgw.usdt2026.cc`。
   - 若有多個 API 實例（負載均衡），需確保 `login_tokens` 表在**同一數據庫**或共享存儲中，且 Bot 請求能落到寫入該 token 的同一實例或可讀到該庫的實例。
   - **多實例時**：可將 `INTERNAL_API_URL` 設為**容器內地址**（如 `http://127.0.0.1:8000`），使 Bot 的請求始終打當前實例，避免負載均衡到無該 token 的實例。

3. **驗證**
   - 重啟 API 後，看啟動日誌中的 `[Bot] INTERNAL_API_URL=...`，應為 `https://tgw.usdt2026.cc`（或你實際對外域名）。
   - 掃碼失敗時看 API 日誌：若出現 `[Bot] Token not found` 且 `INTERNAL_API_URL=...` 與瀏覽器訪問的域名不一致，即為上述配置問題。
