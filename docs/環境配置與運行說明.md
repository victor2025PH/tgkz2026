...
**通過 GitHub Actions 自動配置**：推送到 `main` 分支時，`deploy.yml` 與 `deploy-backend.yml` 會在服務器上自動確保 `/opt/tg-matrix/.env` 含有 `INTERNAL_API_URL=https://tgw.usdt2026.cc` 並重啟 API，無需手動 SSH。手動一鍵配置並重啟（在服務器項目根目錄執行）：
- Linux/Mac: `bash scripts/deploy-bot-env-and-restart.sh`
- Windows PowerShell: `.\scripts\deploy-bot-env-and-restart.ps1`
