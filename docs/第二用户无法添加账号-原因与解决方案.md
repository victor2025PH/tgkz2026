...
5. **「所有用户都不能添加账号」配額統計錯誤**（`backend/core/quota_service.py`）  
   原先在統計 `tg_accounts` 使用量時，對非 local_user 用戶會再加上「未歸屬」帳號數（`owner_user_id` 為空或 `local_user`），導致每個用戶的 used 都被整體未歸屬數量抬高，配額檢查一律超限（如 used ≥ limit），中間件返回 QUOTA_EXCEEDED。修復：僅統計 `owner_user_id = 當前 user_id` 的有效帳號數，不再把未歸屬帳號計入任意用戶的 used。
