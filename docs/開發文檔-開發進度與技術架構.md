...
| service_locator.py | 統一懶加載代理與 get_* 函數（重構後核心） |
...
### v2.1.2 (當前) — main.py 拆分重構與全面錯誤修復

**架構變更：Handler 提取重構**

將 `main.py` 中 553 個 `handle_*` 方法提取到 52 個 `_impl.py` 文件，按領域拆分到 `domain/` 和 `api/handlers/` 目錄。

**新增核心模塊**
- `service_locator.py` — 統一管理所有懶加載模塊代理（`_LazyModuleProxy`）和 `get_*` 工廠函數
  - 14 個懶加載代理：`private_message_poller`、`ai_auto_chat`、`vector_memory`、`resource_discovery` 等
  - 25+ 個 `get_*` 函數：`get_batch_ops()`、`get_multi_role_manager()`、`get_backup_manager()` 等
  - 知識庫單例：`get_knowledge_search_engine()`、`get_document_manager()`、`get_media_manager()`

**修復的導入問題（48 處，涉及 12 個文件）**

| 問題類型 | 影響文件數 | 修復方式 |
|----------|-----------|----------|
| `db` / `config` 缺失 | 1 (session_handlers_impl) | 添加頂層導入 |
| `Path` 缺失 | 2 (api_credential, account) | 添加 `from pathlib import Path` |
| `search_engine` / `document_manager` / `media_manager` 裸引用 | 4 (knowledge, qa, rag, media) | 通過 `service_locator` 的 `get_*` 函數獲取 |
| `DiscoveredResource` 裸引用 | 2 (resource, search) | 通過 `get_DiscoveredResource()` |
| `SpintaxGenerator` 裸引用 | 1 (queue) | 通過 `get_SpintaxGenerator()` |
| `WarmupManager` 裸引用 | 1 (account) | 通過 `get_WarmupManager()` |
| `get_DeviceFingerprintGenerator` / `get_ProxyManager` 缺失 | 1 (account) | 添加到 service_locator 並導入 |
| `get_backup_manager` 缺失 | 1 (backup) | 添加到 service_locator 並導入 |
| `private_message_handler` 缺失 | 1 (monitoring) | 添加懶加載代理並導入 |
| `ai_context` / `resource_discovery` 缺失 | 2 (chat, groups) | 添加到 service_locator 導入 |

**修復的結構性 Bug**
- `handle_save_resource` 死代碼 → 已建立正確的方法委派
- 9 處 `_impl.py` 中的重複函數定義 → 保留正確版本
- 9 處 `main.py` 中的重複方法定義 → 已清理
- `handle_get_initial_state` 錯誤靜默吞掉 → 添加完整的錯誤返回

**驗證工具**
- `scripts/verify_compilation.py` — 靜態編譯驗證（py_compile + import + free var 分析）
- `scripts/test_handlers.py` — P0/P1/P2 運行時 API 測試
- `scripts/deep_scan_impl.py` — AST 深度掃描所有未定義名稱
- 最終結果：**52/52 文件通過，0 編譯錯誤，0 未定義名稱**
...
| main.py 重構後需 Docker 內運行時驗證 | 靜態分析已全部通過，需在容器內跑 P0-P2 測試確認 | 待驗證 |
...
- 在 Docker 容器內運行 `scripts/verify_compilation.py` 驗證全部模塊可導入
- 在 Docker 容器內運行 `scripts/test_handlers.py` 驗證 P0/P1/P2 API 端點
- 逐步將 `self.xxx` 模式遷移到 `ctx.xxx`（service_context 架構演進）
...
```
前端命令 → api/http_server.py → main.py BackendService.handle_*()
                                          ↓ (委派)
                                  domain/*/_impl.py handle_*(self, payload)
                                          ↓ (依賴注入)
                                  service_locator.py → 各業務模塊
```

**_impl.py 文件通用導入模式：**
```python
from service_context import get_service_context       # 所有文件都有
from database import db                                # 需要數據庫的文件
from config import config                              # 需要配置的文件
from error_handler import handle_error, AppError       # 需要錯誤處理的文件
from validators import validate_*, ValidationError     # 需要驗證的文件
from service_locator import (                          # 需要業務服務的文件
    lazy_proxy_var,                                    #   懶加載代理
    get_xxx_function                                   #   工廠/實例獲取函數
)
```

**service_locator.py 核心接口：**

| 類型 | 示例 | 說明 |
|------|------|------|
| 懶加載代理 | `ai_auto_chat`, `resource_discovery` | 首次訪問時加載模塊，直接當作實例使用 |
| 類獲取器 | `get_SpintaxGenerator()`, `get_DiscoveredResource()` | 返回類本身，調用方負責實例化 |
| 實例獲取器 | `get_batch_ops()`, `get_backup_manager()` | 調用底層模塊的 `get_*()` 返回單例實例 |
| 知識庫單例 | `get_knowledge_search_engine()` | 返回 `knowledge_base` 包的模塊級單例 |
