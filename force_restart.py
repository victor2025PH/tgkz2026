...
print('Step 1: Kill stuck build process...')
stdin, stdout, stderr = ssh.exec_command('pkill -f "docker compose build" 2>&1')
...
print('Step 3: Recreate and start all containers...')
stdin, stdout, stderr = ssh.exec_command('cd /opt/tg-matrix && docker compose down && docker compose up -d 2>&1', timeout=120)
print(stdout.read().decode('utf-8', errors='replace'))
...
print('=== API Logs ===')
stdin, stdout, stderr = ssh.exec_command('cd /opt/tg-matrix && docker compose logs --tail=5 api 2>&1')
