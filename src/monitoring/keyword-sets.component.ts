      ...
      <div class="flex items-center justify-between mb-6" [class.mb-4]="embedded()">
        @if (!embedded()) {
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-2xl font-bold" style="color: var(--text-primary);">關鍵詞集管理</h1>
              <p class="text-sm" style="color: var(--text-muted);">管理用於監控匹配的關鍵詞集</p>
            </div>
          </div>
        } @else {
          <div class="text-sm font-medium" style="color: var(--text-secondary);">詞集列表</div>
        }
        <div class="flex items-center gap-3">
          @if (!embedded()) {
