...
账户没有设置 2FA，但界面没有显示验证码输入框，只显示了 2FA 输入框。

## ✅ 解决方案

### 方法 1: 使用脚本删除 session 文件（最简单）

1. **关闭应用**（确保后端进程已停止）

2. **运行清理脚本：**
   ```powershell
   npm run clear-session +639952947692
   ```
   
   或者直接运行：
   ```powershell
   node scripts/clear-session.js +639952947692
   ```
